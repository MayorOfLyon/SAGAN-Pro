...
class Trainer(object):
    ...
    def train(self):
        ...
        print("step_per_epoch is ", step_per_epoch)
        print("number of epoch is ", self.total_step/step_per_epoch)
        ...
        for step in range(start, self.total_step):
            ...
            if (step + 1) % self.sample_step == 0:
                ...
                for i in range(0,78):
                    ...
                    fakes.append(fake_images)
                fakes = np.concatenate(fakes, axis=0)
                fakes = torch.tensor(fakes)
                get_inception_score(fakes)
                for i in range(fakes.shape[0]):
                    # Save each image individually as jpg
                    image_path = os.path.join(self.sample_path, f'fake_{i + 1}.jpg')
                    save_image(denorm(fakes[i, :, :, :].unsqueeze(0)), image_path)
